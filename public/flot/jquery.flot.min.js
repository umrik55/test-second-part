(function () {
  ...
  jQuery.color.parse = function (A) {
    var F,
      H = jQuery.color.make;
    if (
      (F = /rgb\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*\)/.exec(
        A
      ))
    ) {
    ...
    if (
      (F = /rgba\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]+(?:\.[0-9]+)?)\s*\)/.exec(
        A
      ))
    ) {
      return H(
    ...
    if (
      (F = /rgb\(\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*\)/.exec(
        A
      ))
    ) {
      return H(
  ...
  function B(l, W, X, E) {
    ...
    function F(AP, AS) {
      ...
      if (AS.mode == "time") {
        ...
        for (AR = 0; AR < AY.length - 1; ++AR) {
          if (
            AX <
              (AY[AR][0] * AU[AY[AR][1]] + AY[AR + 1][0] * AU[AY[AR + 1][1]]) /
                2 &&
    ...
    function a(AN) {
      ...
      if (AP > 0 && AK > 0) {
        ...
        AM(
          AN.datapoints,
          Math.sin(AQ) * (AP / 2 + AK / 2),
          Math.cos(AQ) * (AP / 2 + AK / 2),
          AN.xaxis,
          AN.yaxis
        );
        Y.lineWidth = AK / 2;
        AM(
          AN.datapoints,
          Math.sin(AQ) * (AP / 2 + AK / 4),
          Math.cos(AQ) * (AP / 2 + AK / 4),
          AN.xaxis,
          AN.yaxis
        );
      }
    ...
    function H(AL, AK, AM) {
      ...
      if (AT) {
        AT.pageX = parseInt(
          AT.series.xaxis.p2c(AT.datapoint[0]) + AN.left + e.left
        );
        AT.pageY = parseInt(
          AT.series.yaxis.p2c(AT.datapoint[1]) + AN.top + e.top
        );
      }
