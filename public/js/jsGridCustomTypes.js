...
function setCarNumLocalType() {
        return {
            ...
            editTemplate: function (value, value2) {
                if (value2.asotrNum == null) {
                    var gArr = [];
                    return this._editAuto = $("<input id='carNumLocEdit' value='" + value + "'>").autocomplete({
                        source: function (request, response) {
                            $.ajax({
                                url: "/api/get-cars-asotr" + request.term + "/" + "NumPe",
                                type: 'GET',
                        ...
                        select: function (e, ui) {
                            $('#carNumGlEdit').val(gArr.find(x => x.NumPe == ui.item.value).NumGov);
                        }
                    });
                } else {
            ...
            editValue: function () {
                return typeof this._editAuto !== "object" ? this._editAuto : this._editAuto.val()
            },
            ...
            filterTemplate: function (value) {
                ...
                this._filterControl = $("<input id='carNumLocFilter' class='autoFind'>").autocomplete({
                    source: function (request, response) {
                        $.ajax({
                            url: "/api/get-cars-asotr" + request.term + "/" + "NumPe",
                            type: 'GET',
                    ...
                    select: function (e, ui) {
                        $('#carNumGlFilter').val(gArrS.find(x => x.NumPe == ui.item.value).NumGov);
                    }
                });
    ...
    function setCarNumGlobalType() {
        return {
            ...
            editTemplate: function (value, value2) {
                if (value2.asotrNum == null) {
                    var gArr2 = [];
                    return this._editAuto = $("<input id='carNumGlEdit' value='" + value + "'>").autocomplete({
                        source: function (request, response) {
                            $.ajax({
                                url: "/api/get-cars-asotr" + request.term + "/" + "NumGov",
                                type: 'GET',
            ...
            filterTemplate: function (value) {
                ...
                this._filterControl = $("<input id='carNumGlFilter'>").autocomplete({
                    ...
                    select: function (e, ui) {
                        $('#carNumLocFilter').val(gArr2S.find(x => x.NumGov == ui.item.value).NumPe);
                    }
                });
        ...
        return {
            ...
            insertTemplate: function (value) {
                ...
                return this._insertAuto = $("<input id='driverNum'>").autocomplete({
                    ...
                    select: function (e, ui) {
                        $('#driverName').val(gArr3.find(x => x.basic_number == ui.item.value).name);
                    }
                });
            },
            insertValue: function () {
            ...
            editTemplate: function (value, value2) {
                if (value2.asotrNum == null) {
                    var gArr3 = [];
                    return this._editAuto = $("<input id='driverNumEdit' value='" + value + "'>").autocomplete({
                        source: function (request, response) {
                            $.ajax({
                                url: "/api/get-drivers-asotr" + request.term + "/" + "basic_number",
                                type: 'GET',
                        ...
                        select: function (e, ui) {
                            $('#driverNameEdit').val(gArr3.find(x => x.basic_number == ui.item.value).name);
                        }
                    });
                } else {
                    return this._editAuto = value;
            ...
            filterTemplate: function (value) {
                ...
                this._filterControl = $("<input id='driverNumFilter'>").autocomplete({
                    ...
                    select: function (e, ui) {
                        $('#driverNameFilter').val(gArr3S.find(x => x.basic_number == ui.item.value).name);
                    }
                });
                this._filterControl.on("keyup", function (e) {
                    grid.search();
    ...
    function setDriverNameType() {
        return {
            ...
            editTemplate: function (value, value2) {
                if (value2.asotrNum == null) {
                    var gArr4 = [];
                    return this._editAuto = $("<input id='driverNameEdit' value='" + value + "'>").autocomplete({
                        source: function (request, response) {
                            $.ajax({
                                url: "/api/get-drivers-asotr" + request.term + "/" + "name",
                                type: 'GET',
                        ...
                        select: function (e, ui) {
                            $('#driverNumEdit').val(gArr4.find(x => x.name == ui.item.value).basic_number);
                        }
                    });
                } else {
                    return this._editAuto = value;
    ...
    function setRouteNameType() {
        return {
            ...
            editTemplate: function (value, value2) {
                if (value2.asotrNum == null) {
                    return this._editControl = $("<input id='routeNameEdit' value='" + value + "'>").autocomplete({
                        source: function (request, response) {
                            $.ajax({
                                url: "/api/get-routes-asotr" + request.term + "/" + "Num",
                                type: 'GET',
            ...
            editValue: function () {
                return typeof this._editControl !== "object" ? this._editControl : this._editControl.val()
            },
            filterTemplate: function (value) {
                var grid = this._grid;
    ...
    function setDatePickerType() {
        return {
            ...
            insertTemplate: function (value) {
                return this._insertPicker = $("<input id='datetimepicker'>").datetimepicker({
                    format: 'd.m.Y', mask: true, closeOnDateSelect: true,
            ...
            editTemplate: function (value, value2) {
                if (value2.asotrNum == null) {
                    return this._editPicker = $("<input id='datetimepickerE'>").datetimepicker({
                        format: 'd.m.Y', mask: true, closeOnDateSelect: true,
                        onSelectDate: function (ct, $i) {
                            //
                        }
                    ...
                    return this._editPicker = new Date(value).customFormat("#DD#.#MM#.#YYYY#");
                }
            },

            filterTemplate: function (value) {
                var grid = this._grid;
                return this._filterPicker = $("<input id='datetimepickerF'>").datetimepicker({
                    format: 'd.m.Y', mask: true, closeOnDateSelect: true,
                    onSelectDate: function (ct, $i) {
                        grid.search();
                    }
    ...
    function setDatePeriodPickerType() {
        ...
        return {
            ...
            filterTemplate: function (value) {
                var grid = this._grid;
                return this._periodfilterPicker = $("<input id='datetimepicker_period'>").daterangepicker({
                    startDate: initDateS,
                    endDate: initDateE,
                    timePicker: true,
            ...
            filterValue: function () {
                ...
                //return this._filterPicker.datetimepicker("getValue") != null? this._filterPicker.datetimepicker("getValue").customFormat("#YYYY#-#MM#-#DD#") : ""
                return {
                    dateS: $('#datetimepicker_period').data('daterangepicker').startDate._d.customFormat("#YYYY#-#MM#-#DD# #hhhh#:#mm#"),
                    dateE: $('#datetimepicker_period').data('daterangepicker').endDate._d.customFormat("#YYYY#-#MM#-#DD# #hhhh#:#mm#")
                }
            },
        };
    }
    ...
    function setNumFuelType() {
        return {
            ...
            editTemplate: function (value, value2) {
                if (value2.asotrNum == null) {
                    return this._editNumFuel = $("<input id='numFuel' value='" + value + "'>");
                } else {
    ...
    function setNumEshlType() {
        return {
            ...
            insertTemplate: function (value) {
                return this._insertNumEshl = $("<input id='numEshl'>").inputmask({mask: "9999-99-99_9999"});
                ;
            },

            editTemplate: function (value, value2) {
                if (value2.asotrNum == null) {
                    return this._editNumEshl = $("<input id='numEshlEdit' value='" + value + "'>").inputmask({mask: "9999-99-99_9999"});
                } else {
            ...
            editValue: function () {
                return typeof this._editNumEshl !== "object" ? this._editNumEshl : this._editNumEshl.val()
            },
    ...
    function SetHyperLinkTypeCar() {
        return {
            ...
            //<a class="iframeFuel" data-fancybox-type="iframe" href="/remains" >Журнал залишків</a>
                var $link = $("<a class='iframeFuelMore'>").attr("id_filial", `${item.id_filial}`).text(item.name_filial);
                return $("<div class='filialMore' data-filial='"+item.id_filial+"'>").append($link);
            },
    ...
    function SetTextAutoFilterTypeCar() {
        return {
            ...
            filterTemplate: function (value) {
                var grid = this._grid;
                this._filterLinkChangerCar = $(`<input id='ChangerFilterCar' value='${JSON.parse($('#scriptID').html()).car}'>`);
    ...
    function calcAllLitres(arrData) {
        ...
        $("#countLit").html("<h1>Кількість літрів за заданим фільтром: <b><font color='red'>" + result + "</font></b></h1>");
        return result;
    }

    function calcAllLitresMore(arrData) {
        var result = arrData.reduce((a, b) => +a + +b.PEfuelOst2, 0);
        $("#countLitMore").html("<h3> | Залишок палива по РО за заданим фільтром: <b><font color='red'>" + result + "</font></b>(л)</h3>");
        return result;
    }
...
//TODO: generate random token for din model
    ...
    function editPickerFixDate(str) {
        ...
        return new Date(date[2] + "-" + date[1] + "-" + date[0] + "T" + time).toISOString();
    }
    ...
    $("body").on("click", ".filialMore", function () {
        ...
        document.getElementById('labelChartFilial').innerHTML = `Всього по: ${this.innerText} за обраний період.`;
